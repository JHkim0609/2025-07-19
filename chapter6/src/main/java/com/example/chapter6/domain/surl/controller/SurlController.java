package com.example.chapter6.domain.surl.controller;

import com.example.chapter6.domain.surl.entity.Surl;
...
public class SurlController {
    ...
    public Surl add(@PathVariable String body, HttpServletRequest req){
        ...
        Surl surl = Surl.builder().id(++surlsLastId).body(body).url(url).build();
        surls.add(surl);
        return surl;
    }
    ...
    public String go(@PathVariable long id) {
        Surl surl = surls.stream().filter(_surl -> _surl.getId() == id).findFirst().orElse(null);
